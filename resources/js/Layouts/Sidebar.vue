...
// We'll emit an event to the parent when the sidebar visibility changes
const emit = defineEmits(['update:sidebarVisibility']);
        ...
        const observer = new MutationObserver((mutationsList) => {
            for (const mutation of mutationsList) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                    ...
                    showingSidebar.value = !isHidden;
                    emit('update:sidebarVisibility', !isHidden); // Emit the updated visibility
                }
            }
        });
        observer.observe(sidebar, { attributes: true });
        showingSidebar.value = !sidebar.classList.contains('-translate-x-full');
        emit('update:sidebarVisibility', showingSidebar.value); // Initial emit
    }
});
</script>
